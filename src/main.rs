...
async fn main() {
    let listeners_and_bots;
    {
        ...
        listeners_and_bots = join_all(bot_tokens.iter().zip(bots).map(|(bot_token, bot)| async {
            (axum_app::start_bot(bot.clone(), bot_token, &stop_token, &senders, &options)
            .await
            .expect("Error while starting bot"), bot)
        })).await;
    }

    join_all(listeners_and_bots.into_iter().map(|(listener, bot)| async {
